...
export default function Leaderboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: picks } = await supabase.from('picks').select('user_id, odds, units, status, profit_loss')
      ...
      const board: LeaderboardEntry[] = profiles.map(profile => {
        ...
        return {
          username: profile.username, totalPicks: userPicks.length, settledPicks: settled.length, wins,
          winrate: settled.length > 0 ? (wins / settled.length) * 100 : 0,
          roi: totalStaked > 0 ? (totalProfit / totalStaked) * 100 : 0,
          totalProfit, avgOdds,
        }
      }).filter(e => e.settledPicks >= MIN_PICKS).sort((a, b) => b.roi - a.roi)
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '100px 24px 60px' }}>
        <div style={{ marginBottom: '40px' }}>
          ...
          <p style={{ color: 'var(--muted)', fontSize: '14px', lineHeight: '1.6' }}>
            Ranked by verified performance. Minimum {MIN_PICKS} settled picks required.<br />
            Every stat is calculated from public, timestamped picks only.
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '6px', marginBottom: '20px' }}>
          {(['roi', 'profit', 'winrate'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{
              ...
              borderColor: filter === f ? 'var(--green-border)' : 'var(--border)', transition: 'all .15s',
            }}>
        ...
        <div style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '16px', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '40px 1fr 70px 70px 80px 90px 80px', gap: '8px', padding: '12px 20px', borderBottom: '1px solid var(--border)' }}>
          ...
          {loading ? (
            ...
            <div style={{ padding: '60px 20px', textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--muted)', fontSize: '13px', marginBottom: '24px' }}>Be the first — post {MIN_PICKS} picks and settle them to appear here.</p>
            ...
            sorted.map((entry, i) => (
              <Link key={entry.username} href={`/u/${entry.username}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block' }}>
                <div
                  style={{ display: 'grid', gridTemplateColumns: '40px 1fr 70px 70px 80px 90px 80px', gap: '8px', padding: '14px 20px', borderBottom: '1px solid var(--border)', transition: 'background .12s', cursor: 'pointer', background: i === 0 ? 'rgba(34,197,94,0.03)' : 'transparent' }}
                  onMouseEnter={e => (e.currentTarget.style.background = 'var(--bg3)')}
                  onMouseLeave={e => (e.currentTarget.style.background = i === 0 ? 'rgba(34,197,94,0.03)' : 'transparent')}
                >
                  <div style={{ fontFamily: 'var(--font-geist-mono)', fontSize: '13px', fontWeight: 500, color: i === 0 ? 'var(--green)' : i === 1 ? '#C0A060' : i === 2 ? '#9090A0' : 'var(--dim)' }}>
                    {i === 0 ? '◈' : i + 1}
                  </div>
                  <div>
                  ...
                  <div style={{ fontFamily: 'var(--font-geist-mono)', fontSize: '13px', alignSelf: 'center' }}>{entry.totalPicks}</div>
                  ...
                  <div style={{ fontFamily: 'var(--font-geist-mono)', fontSize: '13px', fontWeight: 500, alignSelf: 'center', color: entry.roi >= 0 ? 'var(--green)' : 'var(--red)' }}>
        ...
        <div style={{ marginTop: '16px', padding: '14px 18px', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '12px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          {[{ icon: '🔒', text: 'All picks timestamped before kickoff' }, { icon: '≠', text: 'No edits or deletions possible' }, { icon: '◈', text: `Min. ${MIN_PICKS} settled picks to rank` }].map(item => (
